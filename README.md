...
- NIC 優先度の適用対象は**ユーザー単位ではなく端末全体**である
- 判定はイベント発生時点のセッション集合（`quser`）を使って毎回再計算する
- 同一 PC で複数アカウントが同時稼働していても、ユーザーごとに別 NIC を同時優先する機能は提供しない
- つまり、同時ログイン時も端末全体で 1 つの優先経路（Prefer 側 NIC）が選ばれる
