...
- NIC メトリック変更は OS のネットワークスタックに対する端末全体設定として適用される。

### 13.1 非要件（明示）

- 同一 PC 上で複数アカウントが同時稼働する場合に、ユーザー単位で別 NIC を同時優先することは本システムの対象外。
- 同時ログイン時の挙動は「最終ログインユーザー固定」ではなく、イベント時点のセッション集合に対する再評価結果に従う。
